...
<table class="table table-striped">
    ...
    <tbody>
        ...
        function addTps($conn, $kec, $desa, $notps, $peml, $pemp, $duaper, $handle) {

            $query = "INSERT INTO tps ( kec,desa, notps, peml, pemp, duaper, handle ) VALUES (:kec, :desa, :notps, :peml, :pemp, :duaper, :handle)";
        ...
        function updateTps($conn,$id, $kec, $desa, $notps, $peml, $pemp, $duaper, $handle) {
            $query = "UPDATE tps SET kec = :kec, desa = :desa,notps = :notps, peml = :peml, pemp = :pemp, duaper = :duaper, handle = :handle WHERE id = :id";
            $stmt = $conn->prepare($query);
            $stmt->bindParam(':id', $id);
            $stmt->bindParam(':kec', $kec);
            $stmt->bindParam(':desa', $desa);
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_tps'])) {
            ...
            addTps($conn, $kec, $desa, $notps, $peml, $pemp, $duaper, $handle);
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_tps'])) {
            ...
            updateTps($conn, $id,$kec, $desa, $notps, $peml, $pemp, $duaper, $handle);
            header("Location: tps.php");
            exit();
        }
