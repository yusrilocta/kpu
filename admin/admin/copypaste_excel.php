...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    foreach ($data['id_calon'] as $key => $value) {
        ...
        $id_pemilu = $data['id_pemilu'][$key];
        $total = $pemlss + $pempss;

        // Query untuk meng-input data ke tabel "tps"
        $queryTPS = "INSERT INTO tps (id, kec, desa, notps, peml, pemp, duaper) VALUES (?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE kec = VALUES(kec), desa = VALUES(desa), notps = VALUES(notps), peml = VALUES(peml), pemp = VALUES(pemp), duaper = VALUES(duaper);";
        $stmtTPS = $conn->prepare($queryTPS);
        $stmtTPS->bind_param('sssssss',$id_tps, $kec, $desa, $notps, $peml, $pemp, $duaper);
        
        // Query untuk meng-input data ke tabel "calon"
        $queryCalon = "INSERT INTO calon (id, nama_partai, nama_calon, no_urut) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE nama_partai = VALUES(nama_partai), nama_calon = VALUES(nama_calon), no_urut = VALUES(no_urut);";
        $stmtCalon = $conn->prepare($queryCalon);
        $stmtCalon->bind_param('ssss',$id_calon, $nama_partai, $nama_calon, $no_urut);
        
        // Query untuk meng-input data ke tabel "pemilu"
        
        $queryPemilu = "INSERT INTO pemilu (id, id_calon, id_tps, peml, pemp, total) VALUES (?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE id_calon = VALUES(id_calon), id_tps = VALUES(id_tps), peml = VALUES(peml), pemp = VALUES(pemp), total = VALUES(total)";
        $stmtPemilu = $conn->prepare($queryPemilu);
        $stmtPemilu->bind_param('ssssss', $id_pemilu, $id_calon, $id_tps, $pemlss, $pempss, $total);
...
<body class="container">
    ...
    <form id="pasteForm">
        <div class="form-group">
            <textarea class="form-control mt-3" id="pasteData" name="pasteData" rows="6" placeholder="Tempelkan data di sini"></textarea>
        </div>
        <button type="button" onclick="parseAndPasteData()" class="btn btn-primary mt-2">Tempelkan Data</button>
    </form>
    ...
    lines.forEach(line => {
        const columns = line.split('\t'); // Menggunakan spasi sebagai pemisah
